...
export class JogoView {
    ...
    */
    ...
    criarCard(celula, index, onClick) {
        ...
        cardWrapper.addEventListener("click", () => onClick(index, celula, img, cardWrapper));
    ...
    atualizarCard(celula, imgElement) {
        imgElement.src = celula.isVisivel() ? celula.getCaminho() : celula.getVerso();
    }
    ...
    atualizarTempo(segundos) {
        if (this.tempoElement) {
            ...
            this.tempoElement.textContent = `${min}:${sec.toString().padStart(2, "0")}`;
        }
    }
